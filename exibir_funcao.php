<?php
  ...
  if(isset($_GET['id'])){
    $id = $_GET['id'];
    $objeto = new funcao();
    $resultado = $objeto->apagarFuncao($id);
  }
?>
<!DOCTYPE html>
<html lang="pt-br">
            ...
            <table class="table table-hover table-bordered mt-5" style="background:white">
                <thead class="thead-dark">
                    <tr>
                        ...
                        <th scope="col" style="vertical-align:middle">Id_Cbo</th>
                        <th scope="col" style="vertical-align:middle">EPIs</th>
                ...
                ?>
                        <tr>
                            <td><?=$registro->nome?></td>
                            <td style="text-align: justify;"><?=$registro->descricao?></td>
                            <td><?=$registro->id_cbo?></td>
                            <td><?=$registro->id_epi?></td>
                            <?php
                            if(!($acesso == "disabled")){
                            ?>
                                <td class="text-center"><a href='editar_funcao.php?id=<?=$registro->id?>'><i class='fas fa-edit'></i></a></td>
                                <td class="text-center"><a href='exibir_funcao.php?id=<?=$registro->id?>'><i class='fa fa-trash' aria-hidden='true'></i></a></i></td>
                            <?php
                            }
                            ?>
                        </tr>
