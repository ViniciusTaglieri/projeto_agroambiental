<?php
  ...
  if(isset($_GET['id'])){
    $id = $_GET['id'];
    $objeto = new riscos();
    $resultado = $objeto->apagarRiscos($id);
  }
?>
<!DOCTYPE html>
<html lang="pt-br">
            ...
            <table class="table table-hover table-bordered mt-5" style="background:white">
                <thead class="thead-dark">
                    <tr>
                        ...
                        <th scope="col" style="vertical-align:middle">Subcategoria</th>
                ...
                ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                    if($registro->id_subcategoria){
                                        $resultado = $subcategoria->selecionarSubcategoriaWhere($registro->id_subcategoria);
                                        foreach($resultado as $nomeSubcategoria){}
                                        echo $nomeSubcategoria->nome;
                                    }
                                ?>
                            </td>
                            <?php
                            if(!($acesso == "disabled")){
                            ?>
                            <td class="text-center"><a href='editar_riscos.php?id=<?=$registro->id?>'><i class='fas fa-edit'></i></a></td>
                            <td class="text-center"><a href='exibir_riscos.php?id=<?=$registro->id?>'><i class='fa fa-trash' aria-hidden='true'></i></a></i></td>
                            <?php
                            }
                            ?>
                        </tr>
