...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $objeto = new MedidasLocal();
    $resultado = $objeto->apagarTitulo($id);
}
?>
<!DOCTYPE html>
<html lang="pt-br">
            ...
            <div class="col-lg-8 table-responsive py-4">
                ...
                <table class="table table-hover table-bordered mt-5" style="background:white">
                    ...
                    <tbody>
                        ...
                        $lista_titulo = $obj_medidas->selecionarTitulo();
                        foreach ($lista_titulo as $titulo) {
                        ?>
                            <tr>
                                <td>
                                    <?php
                                    $lista_local = $obj_local->selecionarLocalWhere($titulo->id_local);
                                ...
                                if (!($acesso == "disabled")) {
                                ?>
                                    <td class="text-center"><a href='exibir_medida_local.php?id=<?= $titulo->id ?>'><i class='fa fa-trash' aria-hidden='true'></i></a></i></td>
                                <?php
                                }
                                ?>
                            </tr>
